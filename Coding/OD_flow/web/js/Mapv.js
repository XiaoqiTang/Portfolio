...
util.extend(Layer.prototype, {
    ...
    wgs2gcj : function (wgsLat, wgsLon) {
        ...
        return {'lat' : (parseFloat(wgsLat) + d.lat),'lon' : (parseFloat(wgsLon) + d.lon)};
    },
    //GCJ-02 to WGS-84
    ...
    gcj2wgs_exact : function (gcjLat, gcjLon) {
        var initDelta = 0.01;
        ...
        while (1) {
            wgsLat = (mLat + pLat) / 2;
            wgsLon = (mLon + pLon) / 2;
            var tmp = this.wgs2gcj(wgsLat, wgsLon)
            dLat = tmp.lat - gcjLat;
            dLon = tmp.lon - gcjLon;
    ...
    // mercatorLat -> y mercatorLon -> x
    mercator2wgs : function(mercatorLat, mercatorLon) {
    ...
    transformLon : function (x, y) {
        ...
        for (var j = 0; j < data.length; j++) {
            ...
            if (data[j].geo) {
                ...
                }else if (this.getCoordType() === 'wgs84') {
                    for (var i = 0; i < data[j].geo.length; i++) {
                        var gcj=GPS.wgs2gcj(data[j].geo[i][1],data[j].geo[i][0]);
                        var bd=GPS.gcj2bd(gcj.lat,gcj.lon);
                        // console.log(gcj);
                        var pixel = map.pointToPixel(new BMap.Point(bd.lon, bd.lat));
                        tmp.push([pixel.x, pixel.y, parseFloat(data[j].geo[i][2])]);
                    }
                }
                data[j].pgeo = tmp;
